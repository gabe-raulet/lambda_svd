...
namespace FMI {
    ...
    public:
        ...
        void send_raw(void* ptr, std::size_t bytes, FMI::Utils::peer_num dest) {
            std::string channel = policy->get_channel({Utils::send, bytes});
            channel_data data {(char *) ptr, bytes};
            channels[channel]->send(data, dest);
        }
        ...
        void recv_raw(void* ptr, std::size_t bytes, FMI::Utils::peer_num src) {
            std::string channel = policy->get_channel({Utils::send, bytes});
            channel_data data {(char *) ptr, bytes};
            channels[channel]->recv(data, src);
        }

        //! ISend buf to peer dest
        template<typename T>
        void isend(Comm::Data<T> &buf, FMI::Utils::peer_num dest) {
            std::string channel = policy->get_channel({Utils::send, buf.size_in_bytes()});
            channel_data data {buf.data(), buf.size_in_bytes()};
            channels[channel]->isend(data, dest);
        }

        //! IReceive data from src and store data into the provided buf
        template<typename T>
        void irecv(Comm::Data<T> &buf, FMI::Utils::peer_num src) {
            std::string channel = policy->get_channel({Utils::send, buf.size_in_bytes()});
            channel_data data {buf.data(), buf.size_in_bytes()};
            channels[channel]->irecv(data, src);
        }
        ...
        void scatter_raw(void* send_ptr, std::size_t send_bytes, void* recv_ptr, std::size_t recv_bytes, FMI::Utils::peer_num root) {
            std::string channel = policy->get_channel({Utils::scatter, recv_bytes});
            channel_data senddata {(char *) send_ptr, send_bytes};
            channel_data recvdata {(char *) recv_ptr, recv_bytes};
            channels[channel]->scatter(senddata, recvdata, root);
        }
