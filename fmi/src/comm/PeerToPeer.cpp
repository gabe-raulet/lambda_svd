...
void FMI::Comm::PeerToPeer::isend(channel_data buf, FMI::Utils::peer_num dest) {
    isend_object(buf, dest);
}

void FMI::Comm::PeerToPeer::irecv(channel_data buf, FMI::Utils::peer_num src) {
    irecv_object(buf, src);
}
